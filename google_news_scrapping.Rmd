...
You can notice in the right side of the screen snapshot that we used the **dev-tools** in Google Chrome in order to detect all the *html nodes*. You can access this tool by hitting the **F12** key. The html nodes are passed as arguments into the **rvest** functions.

Basically, the idea is to extract the communication vehicle, the time elapsed since the news was published, and the main headline.

The code used are presented below: 
...
In this last case we have used a regular expression (REGEX) to clean the data by separating the actual highlight phrases from the complementary phrases. IN some cases we have a phrase ending in uppercase letters such as "SP" collapsed with other phrase initiating in "A" for example. We have to think of a better way to split these cases, but the current result is quite satisfactory for now. 
